...
export default {
  ...
  interceptor: response => {
    ...
    // 接口返回处理
      ...
      axiosResponseData.code = parseInt(String(axiosResponseData.code), 10);

      const { code, message, msg, sub_msg, data: content } = axiosResponseData;
      const validMessage = message | sub_msg || msg || 'unknow error';
      let extraMessage = '';
      ...
      if (code !== StatusCode.SUCCESS) {
        ...
        if (code === StatusCode.NOSESSION || code === StatusCode.TOCKEN_INVALID) {
          ...
          Vue.http.cancel(`【${code}】Request was canceled（${validMessage}）${extraMessage}`);
        ...
        throw new ServiceError(`${validMessage}${extraMessage}`, code);
      }
    }
